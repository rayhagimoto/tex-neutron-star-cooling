...
To use this method we rewrite the integrand by making the replacements:\todo{This is incorrect. In particular \eref{eq:energy-replacement-formula} doesn't work because the thermal distributions $f$ are not Lorentz invariant. The $E_i$ that shows up in \eref{eq:emissivity-integral} is the energy \emph{as seen in the rest frame of the NS}. Furthermore, \eref{eq:p2-dot-p3-replacement-rule} was derived using incorrect assumptions that are not corrected in the appendix.}
\begin{align}
...
\end{align}
where $\Lambda_i$ and $\lambda_i$ are defined in \eref{eq:app-lambda-definitions}.
...
\begin{figure}[htbp]
    ...
    \caption{
        Illustration of sampling from the physical region of parameter space dictated by the $\Theta(-G_i)$ step-functions in \eref{eq:recursive-phase-space-with-momentum-transfer-for-n=3}. 
        \emph{(Left:)}
